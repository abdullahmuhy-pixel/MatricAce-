...
self.addEventListener('fetch', e => {
  ...
  // Never intercept non-GET requests (logins, writes, etc). Caching only
  // ever applies to GET; if a POST's network fetch fails, caches.match()
  // can never resolve it, which turns a transient error into a hard
  // "Failed to fetch" instead of letting the real error surface.
  ...
  // Network-first for the HTML shell itself: always serve the latest deploy
  // when online (this is the piece that was causing testers to see stale
  // code after every redeploy); fall back to the cached copy only when
  // offline. Static, rarely-changing assets (icon, manifest) keep the
  // instant-load stale-while-revalidate behaviour below.
  if (url.endsWith('student-portal.html') || url.endsWith('/app/') || e.request.mode === 'navigate') {
    e.respondWith(
      fetch(e.request).then(res => {
        if (res.ok) {
        ...
        return res;
      }).catch(() => caches.open(CACHE_V).then(cache => cache.match(e.request)))
    );
    return;
  }
  // Stale-while-revalidate for the remaining app shell/static assets: serve the cached
...
self.addEventListener('notificationclick', e => {
  ...
  const targetUrl = (e.notification.data && e.notification.data.url) || './student-portal.html';
  e.waitUntil(
